...
Following Table~\ref{tab:model_hyperparams} outlines the core architectural configurations of Nebium Transformer.

\begin{table}[H]
\centering
\small
\caption{Detailed architectural specifications and hyperparameter settings for Nebium.}
\label{tab:model_hyperparams}
\begin{tabularx}{\linewidth}{l l X}
\toprule
\textbf{Hyperparameter} & \textbf{Symbol / Value} & \textbf{Architectural Justification} \\
\midrule
Vocabulary Size & $V = 5,000$ & General purpose BPE tokenizer does properly represent the Chess notations \\
Embedding Dimension & $d_{\text{model}} = 512$ & Balanced latent space for representing complex board moves \\
Number of Layers & $L = 6$ & Sufficient hierarchical depth for multi-play tactical reasoning \\
Attention Heads & $h = 8$ & Parallel subspace projections ($d_k = d_{\text{model}}/h = 64$) \\
FFN Expansion Dim & $d_{ff} = 1,368$ & SwiGLU 2/3 ratio preserving parameter parity with standard FFN \\
Positional Encoding & Rotary (RoPE) & Enables exact relative move-distance encoding and length extrapolation \\
Activation Function & SwiGLU & Enhances multiplicative gating and non-vanishing gradients \\
Layer Normalization & RMSNorm & Invariant scaling without mean centering improves training stability and efficiency \\
Dropout Rate & $p_{\text{drop}} = 0.10$ & Regularization preventing overfitting for opening repetitions which are repetitive in chess games \\
Max Sequence Length & $T_{\text{max}} = 512$ & Supports complete chess games up to 256 full moves (512 plies) \\
Total Parameters & 28.4 Million & Lightweight footprint suitable for low-latency desktop execution \\
\bottomrule
\end{tabularx}
\end{table}
...
Standard transformer architectures utilize a two-layer Multi-Layer Perceptron (MLP) with Gaussian Error Linear Units (GELU) \parencite{vaswani2017attention}. In contrast, Nebium adopts the Swish-Gated Linear Unit (SwiGLU) \parencite{shazeer2020glu}, which introduces bilinear multiplicative gating to regulate information flow. Formally, for an intermediate hidden representation $\mathbf{x} \in \mathbb{R}^{d_{\text{model}}}$:
\begin{equation}
...
\end{equation}
where $\text{SiLU}(z) = z \cdot \sigma(z) = \frac{z}{1 + e^{-z}}$, and $\odot$ denotes element-wise Hadamard multiplication. The projection matrices are parameterized as $\mathbf{W}_{\text{gate}}, \mathbf{W}_{\text{up}} \in \mathbb{R}^{d_{\text{model}} \times d_{ff}}$ and $\mathbf{W}_{\text{down}} \in \mathbb{R}^{d_{ff} \times d_{\text{model}}}$.

Because SwiGLU incorporates three projection matrices rather than two, maintaining parameter parity with a conventional FFN ($4 \cdot d_{\text{model}}$) requires scaling the hidden dimension to approximately $\frac{2}{3} \cdot 4 d_{\text{model}}$. In Nebium, $d_{ff}$ is computed as the nearest multiple of 8:
\begin{equation}
    d_{ff} = 8 \times \left\lfloor \frac{\frac{2}{3} \times 4 \times d_{\text{model}} + 7}{8} \right\rfloor = 8 \times \left\lfloor \frac{1365.33 + 7}{8} \right\rfloor = 1,368
\end{equation}
This architectural adjustment ensures identical parameter count while delivering richer gradient pathways and accelerated empirical convergence.

\subsection{Root Mean Square Layer Normalization (RMSNorm)}
\label{sec:rmsnorm_method}
Standard Layer Normalization enforces zero-mean and unit-variance across hidden feature dimensions:
\begin{equation}
...
\end{equation}
Empirical evidence indicates that the variance scaling property accounts for nearly all stability gains, while the mean-centering step is computationally redundant \parencite{zhang2019root}. Nebium integrates RMSNorm, which modifies the transformation by scaling purely by the root mean square:
\begin{equation}
...
\end{equation}
By eliminating mean computation, RMSNorm achieves a 15--25\% speedup per layer and stabilizes pre-layer normalized training dynamics under half-precision FP16 operations.

\subsection{Inference Engine and Dynamic Legal Move Masking}
\label{sec:legal_masking}
During inference, Nebium Model takes a prefix of tokens(chess moves) as input which represents previous moves $\mathbf{x}_{<t}$. Then model emits raw unnormalized logits $\mathbf{z}_t \in \mathbb{R}^V$ as output. This project uses the unconstrained generation  via greedy selection for measuring the latent learning but for competitive game play legal move masking has used.

For bridging the sequence modeling and physical game rules, Nebium model implements a \textbf{Dynamic Legal Move Masking} inference protocol. The current board state $s_t$ is synchronized using \texttt{python-chess} \parencite{romstad2024pythonchess}. The set of physically legal moves in coordinate format $\mathcal{M}_{\text{legal}}(s_t)$ is mapped into corresponding token indices $\mathcal{I}_{\text{legal}} \subset \{1, \dots, V\}$. A binary filter mask $\mathbf{M}_{\text{legal}} \in \{0, 1\}^V$ is constructed, and non-legal logits are clamped to negative infinity:
...
where $\tau > 0$ represents sampling temperature. The final probability distribution is obtained via softmax:
\begin{equation}
    P(x_t = i \mid \mathbf{x}_{<t}) = \frac{\exp(\tilde{z}_{t, i})}{\sum_{j \in \mathcal{I}_{\text{legal}}} \exp(\tilde{z}_{t, j})}
\end{equation}
This hybrid architecture combines the strategic intuition and fast pattern recognition of deep transformers with the absolute correctness of rule verification.

To transition from training to low-latency edge serving, Nebium adopts a PyTorch-to-GGUF serialization pipeline rather than legacy ONNX Runtime or TensorFlow Serving frameworks. Whereas traditional ONNX exports decouple computation graphs from tokenizer vocabularies and suffer from Python interpreter bottlenecks during sequential decoding, GGUF encapsulates model weights, block-wise quantization scales (\texttt{Q4\_K\_M}), and BPE metadata into a unified binary, enabling zero-copy memory-mapped inference on commodity CPUs (detailed in Section~\ref{sec:tools_deployment}).
